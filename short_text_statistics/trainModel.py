...
class Dataset(object):
    ...
    def _genVocabulary(self, reviews, labels):
        ...
        # """将词频数据写入文件"""
        # test_sortWordCount = os.path.join(basePath, 'data/sortWordCount.json')
        # utils.removeFileIfExists(test_sortWordCount)
        # with open(test_sortWordCount, "w", encoding="utf-8") as f:
        #     json.dump(sortWordCount, f, ensure_ascii=False)
        ...
        utils.removeFileIfExists(f_word2idx_json)
        utils.removeFileIfExists(f_label2idx_json)
...
class WordVector:
    ...
    def __init__(self, wordVectorParam):
        self.wordVectorParam = wordVectorParam

    def generateWord2VectorFile(self):
        _sourceFile = file_labeled_train_data_csv
        _word2VecFile = bin_word2Vec
        """
        ...
        utils.removeFileIfExists(temp_file)
        utils.removeFileIfExists(_word2VecFile)
        ...
        self.generateWord2Vector(_word2VecFile)

    def generateWord2Vector(self,_word2VecFile):
        """
        根据语料库文件生成词向量文件
        ...
        """
            设置参数
        """
        wordVectorParam_dict = self.wordVectorParam.__dict__
        # vec_sentences = wordVectorParam_dict.get('sentences')
        # vec_corpus_file = wordVectorParam_dict.get('corpus_file')
        # vec_size = wordVectorParam_dict.get('size')
        vec_window = wordVectorParam_dict.get('window')
        vec_min_count = wordVectorParam_dict.get('min_count')
        vec_sg = wordVectorParam_dict.get('sg')
        vec_cbow_mean = wordVectorParam_dict.get('cbow_mean')
        vec_hs = wordVectorParam_dict.get('hs')
        vec_negative = wordVectorParam_dict.get('negative')
        vec_ns_exponent = wordVectorParam_dict.get('ns_exponent')
        vec_iter = wordVectorParam_dict.get('iter')
        vec_alpha = wordVectorParam_dict.get('alpha')
        vec_min_alpha = wordVectorParam_dict.get('min_alpha')
        vec_hashfxn = wordVectorParam_dict.get('hashfxn')
        vec_null_word = wordVectorParam_dict.get('null_word')
        vec_trim_rule = wordVectorParam_dict.get('trim_rule')
        vec_sorted_vocab = wordVectorParam_dict.get('sorted_vocab')
        vec_workers = wordVectorParam_dict.get('workers')
        vec_batch_words = wordVectorParam_dict.get('batch_words')
        vec_max_final_vocab = wordVectorParam_dict.get('max_final_vocab')
        vec_max_vocab_size = wordVectorParam_dict.get('max_vocab_size')
        vec_seed = wordVectorParam_dict.get('seed')
        vec_sample = wordVectorParam_dict.get('sample')
        vec_compute_loss = wordVectorParam_dict.get('compute_loss')
        vec_callbacks = wordVectorParam_dict.get('callbacks')
        ...
        model = gensim.models.Word2Vec(sentences,
                                       ...
                                       sg=vec_sg,
                                       iter=vec_iter,
                                       cbow_mean=vec_cbow_mean,
                                       hs=vec_hs,
                                       negative=vec_negative,
                                       ns_exponent=vec_ns_exponent,
                                       alpha=vec_alpha,
                                       min_alpha=vec_min_alpha,
                                       hashfxn=vec_hashfxn,
                                       null_word=vec_null_word,
                                       trim_rule=vec_trim_rule,
                                       sorted_vocab=vec_sorted_vocab,
                                       workers=vec_workers,
                                       batch_words=vec_batch_words,
                                       max_final_vocab=vec_max_final_vocab,
                                       max_vocab_size=vec_max_vocab_size,
                                       seed=vec_seed,
                                       sample=vec_sample,
                                       compute_loss=vec_compute_loss
                                       )
...
class TrainingModel:
    ...
    def trainingModel(self):
        ...
        log.info("train data shape: {}".format(data.trainReviews.shape))
        log.info("train label shape: {}".format(data.trainLabels.shape))
        log.info("eval data shape: {}".format(data.evalReviews.shape))
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                for i in range(config.training.epoches):
                    # 训练模型
                    log.info("epoches：{}{}".format(i, config.training.epoches))
                    for batchTrain in nextBatch(trainReviews, trainLabels, config.batchSize):
                        loss, acc, prec, recall, f_beta = trainStep(batchTrain[0], batchTrain[1])

                        currentStep = tf.train.global_step(sess, globalStep)
                        log.info("train: step: {}, loss: {}, acc: {}, recall: {}, precision: {}, f_beta: {}".format(
                        ...
                        if currentStep % config.training.evaluateEvery == 0:
                            ...
                            log.info("{}, step: {}, loss: {}, acc: {},precision: {}, recall: {}, f_beta: {}".format(time_str,currentStep,mean(losses),mean(accs),mean(precisions),mean(recalls),mean(f_betas)))
